...
for (var i = 0; i < c2.length; i++) {
  ...
  for (var j = 0; j < c2[i].length; j++) {
    ...
    if (c2[i][j] == c1[0][0] && c2[i][j + 1] == c1[0][1] && c2[i][j + 2] == c1[0][2]) {
      // if c2[1][2] (the first char of the first element of the array c2) == c1[0][0] (the first char of the first element of the array c1)
      // and c2[1][3] == c1[0][1]
      // and c2[1][4] == c2[0][2]
      // then continue to check

      if (c2[i + 1][j] == c1[1][0] && c2[i + 1][j + 1] == c1[1][1] && c2[i + 1][j + 2] == c1[1][2]) {
        // if c2[2][2] (the first char of the 2nd element of the array) == c1[1][0] (the first char of the 2nd element of the array)
        // and c2[2][3] == c1[1][1]
        // and c2[2][4] == c1[1][2]
        // then continue to check

        if (c2[i + 2][j] == c1[2][0] && c2[i + 2][j + 1] == c1[2][1] && c2[i + 2][j + 2] == c1[2][2]) {
          // if c2[3][2] (the first char of the 3rd element of the array) == c1[2][0] (the first char of the 3rd element of the array)
          // and c2[3][3] == c1[2][1]
          // and c2[3][4] == c1[2][2]
          // then RETURN (2, 1) == c2[1][2] == c2[i][j]
          console.log(j + "," + i);
